...
<div class="aiz-titlebar mt-2 mb-4">
    <div class="row align-items-center">
        <div class="col-md-6">
            ...
            $is_primium = \App\Models\User::where('id', $user_id)->pluck('is_premium')->first();
        @endphp
            <h1 class="h3 ">{{ translate('Dashboard') }}

                @if($is_primium==0)
                {{-- <a type="button" data-toggle="modal" data-target="#editModal"
                    >Get Primium Membership</a> --}}
                    <button type="button" data-toggle="modal" data-target="#editModal">
                        Get Premium Membership
                    </button>
                {{-- <button type="button" onclick="showAddToCartModal(9100)">

                </button> --}}
                {{-- <a href="{{ route('checkout.purchase_membership_info') }}"><span style="color: blue; font-weight: bold"> Get Primium Membership</span></a> --}}
                @endif
            </h1>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="editModalLabel">Premium Membership Feture</h5>
                            ...
                            <form
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary"
                                        ...
                                        <button type="button" onclick="buyMembershipNow()">
                                            Proceed to Checkout
                                        </button>
                                </div>
                            </form>
    ...
    <div class="col-md-6">
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0 mt-0">{{ translate('Default Shipping Address') }}</h6>
            </div>
            <div class="card-body">
...
function showAddToCartModal(id){
    ...
    function buyMembershipNow(){
        ...
        if(checkAddToCartValidity()) {
            ...
            $.ajax({
               type:"POST",
               url: 'http://51.91.204.58/cart/addtocart',
               ...
               success: function(data){
                   if(data.status == 1){
                        ...
                        window.location.replace("http://51.91.204.58/checkout/membership_info?premium=1");
                   }
                   else{
                        $('#addToCart-modal-body').html(null);
                        $('.c-preloader').hide();
