...
<div class="modal-body gry-bg px-3 pt-3">
    ...
    <div class="card mt-4">
        <div class="card-header">
          <b class="">{{ translate('Order Summary') }}</b>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-lg-6">
    ...
    <div class="row">
        <div class="col-lg-9">
            <div class="card mt-4">
                <div class="card-header">
                  <b class="">{{ translate('Order Details') }}</b>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($orderDetail->product != null && $orderDetail->product->auction_product == 0)
                                            <a href="{{ route('product', $orderDetail->product->slug) }}" target="_blank">{{ $orderDetail->product->getTranslation('name') }} @if ($orderDetail->cancel == 1) - <span style="color:red;"> Cancelled</span> @endif</a>
                                        ...
                                        @endif
                                        @if ($status == 'delivered' && $orderDetail->cancel == 0) 
										<span style="background: #e5ec0a;padding: 3px 10px;border: 1px solid #b3900a;">
                                           <a onclick="makereview({{ $orderDetail->product->id }})" data-toggle="modal" data-target="#makereview" href="javascript:void(0)">Review </a>
										</span>  
										@endif
                                    </td>
                                    <td>
        ...
        <div class="col-lg-3">
            <div class="card mt-4">
                <div class="card-header">
                  <b class="">{{ translate('Order Ammount') }}</b>
                </div>
                    ...
                    <table class="table table-borderless">
                        <tbody>
							...
							@if ($order->orderDetails->whereIn('cancel','1')->count())
								<tr>
                                <td class="w-50 fw-600">{{ translate('Cancelled')}}</td>
                                <td class="text-right">
                                    <span class="strong-600">- {{ single_price($order->orderDetails->whereIn('cancel', '1')->sum('price')) }}</span>
                                </td>
								</tr>
							@endif
                            <tr>
...
<script type="text/javascript">
    ...
    function makereview(order_id){
            document.querySelector("#product_id").value = order_id;
    }
</script>
