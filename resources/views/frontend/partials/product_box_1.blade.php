<div class="aiz-card-box border border-light rounded hov-shadow-md mt-1 mb-2 has-transition bg-white">
    ...
    <div class="position-relative">
        <a href="{{ route('product', $product->slug) }}" class="d-block">
			...
			@else
				@php
					$productglarry = explode(',', $product->photos);
					if(is_array($productglarry)){
						$imagethumb = $productglarry[0] ?? null;
					}else{
						$imagethumb = $productglarry;
					}
				@endphp
				@if (!empty($imagethumb))
					<img
        ...
        @if ($product->wholesale_product)
            <span class="absolute-bottom-left fs-24 text-white fw-600 px-2 lh-1-8" style="background-color: #455a64">
                {{ translate('Wholesale') }}
            </span>
        @endif
        <div class="absolute-top-right aiz-p-hov-icon">
            ...
            <a href="javascript:void(0)" class="d-none" onclick="addToCompare({{ $product->id }})" data-toggle="tooltip" data-title="{{ translate('Add to compare') }}" data-placement="left">
    ...
    <div class="p-md-3 p-2 text-left">
        ...
        <h3 class="fw-600 fs-24 text-truncate-2 lh-1-4 mb-0 h-35px">
