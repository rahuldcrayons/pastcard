...
@if ($is_premium_buy==0)
                    ...
                    <div class="row aiz-steps arrow-divider">
                        ...
                        <div class="col active">
                            <div class="text-center text-primary">
                                ...
                                <h3 class="fw-600 d-none d-lg-block">{{ translate('4. Payment') }}</h3>
                            </div>
                        </div>
                        <div class="col">
    ...
    <section class="mb-4">
        <div class="container text-left">
            <div class="row">
                <div class="col-lg-8 order-sm-2 order-md-2 order-lg-1 order-2">
                    <form action="{{ route('payment.checkout') }}" class="form-default" role="form" method="POST"
                        ...
                        <input type="hidden" name="shipping_final" value="{{ $shipping_final }}">
                                                        ...
                                                        <div class="col-6 col-md-4">
                                                            <label class="aiz-megabox d-block mb-3">
                                                                <input value="cash_on_delivery" class="online_payment"
                                                                    id="cod" type="radio" name="payment_option">
                                                                <input type="text" name="codselect" id="codselect"
                                                                    value="">
                                                                <span class="d-block p-3 aiz-megabox-elem">
                                    ...
                                    <div class="text-center py-4">
                                        ...
                                        @if (Auth::user()->balance < $total && Auth::user()->balance > 0)
                                            <div class="form-switch">
                                                <span style="color:#0d6efd; font-weight: bold">Use wallet balance</span>
                                                <input class="form-check-input" type="checkbox" role="switch"
                                                    name="include_wallet" id="include_wallet"
                                                    style="margin-left: 5px !important;">
                                            </div>
                                        @elseif (Auth::user()->balance < $total)
                        ...
                        <div class="row align-items-center pt-3">
                            ...
                            <div class="col-6 text-right">
                                <button type="button" onclick="submitOrder(this)" id="checkoutbtn"
                ...
                <div class="col-lg-4 order-1 order-sm-1 order-md-1 order-lg-2 mt-4 mt-lg-0" id="cart_summary">
                    @include('frontend.partials.cart_summary')
                </div>
            </div>
        </div>

        <div class="modal" id="codmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
            aria-modal="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <form class="form-default" id="verifyotp" method="POST">
                        @csrf
                        <div class="modal-body">
                            ...
                            <form action="javascript: void(0)" class="otp-form" name="otp-form">
                                ...
                                <div class="title">
                                    <p class="info">An otp has been sent to your number</p>
                                    <p class="msg">Please enter OTP to verify</p>
                                </div>
                                <div class="otp-input-fields">
                                    <input type="number" name="1" required
                                        class="otp__digit otp__field__1 otpfrm">
                                    <input type="number" name="2" required
                                        class="otp__digit otp__field__2 otpfrm">
                                    <input type="number" name="3" required
                                        class="otp__digit otp__field__3 otpfrm">
                                    <input type="number" name="4" required
                                        class="otp__digit otp__field__4 otpfrm">
                                    <input type="number" name="5" required
                                        class="otp__digit otp__field__5 otpfrm">
                                    <input type="number" name="6" required
                                        class="otp__digit otp__field__6 otpfrm">
                                </div>
                            ...
                            <div class="p-3">
                                ...
                                <div class="form-group text-center">
                                    <button type="submit" id="otpverify" class="btn btn-primary fw-600">Verify</button>
                                </div>
                            </div>
                        </div>
                    </form>
...
@section('script')
    <script type="text/javascript">
		...
		$(document).ready(function() {
			$('#include_wallet').click(function() {
			var totalamnt = <?php echo $total; ?>;
			var totalamnt1 = totalamnt.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,");
				if ($(this).is(':checked')) { 
				var usrbalance = <?php echo Auth::user()->balance ?>;
				var calremaining = totalamnt - usrbalance;
				var finalamt = calremaining.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,");
				$('#finaltotal').html('Rs '+finalamt);
				}else{
				$('#finaltotal').html('Rs '+totalamnt1);
				}
			});
		});
        ...
        $(document).on("click", "#cod", function() {
            $.ajax({
                ...
                method: "POST",
                url: "{{ route('checkout.codotp') }}",
                cache: false,
                contentType: false,
                processData: false,
                success: function(data) {
                    $('#codselect').attr('value', '1');
                    $("#checkoutbtn").prop("disabled", true);
                    $('#verifyotp').trigger("reset");
                    $('#codmodal').modal('show');
                }
            })
        })
        ...
        $(document).on("click", "#otpverify", function() {
            var data = new FormData($('#verifyotp')[0]);
