	...
	<div>
		...
		<div style="background: #eceff4;padding: 1rem;">
			...
			<table>
				...
				<tr>
					<td style="font-size: 1rem;" class="strong">{{ translate('GST No.') }}: {{ get_setting('gst_no') }}</td>
					<td class="text-right"></td>
				</tr>
				<tr>
	                ...
	                @foreach ($order->orderDetails as $key => $orderDetail)
		                @if ($orderDetail->product != null)
							<tr class="">
								...
								<td>{{ $orderDetail->product->name }} @if($orderDetail->variation != null) ({{ $orderDetail->variation }}) @endif - <span style="color: red;"> Cancelled</span></td>
								@else
			            ...
			            <td>
					        <table class="text-right sm-padding small strong">
						        <tbody>
									...
									<tr>
							            <th class="gry-color text-left">{{ translate('Cancelled') }}</th>
							            <td class="currency">- {{ single_price($order->orderDetails->whereIn('cancel', '1')->sum('price')) }}</td>
							        </tr>
									@endif
									<tr>
