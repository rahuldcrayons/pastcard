...
echo "=== WORDPRESS TO LARAVEL PRODUCT MIGRATION ===\n\n";
...
if (($handle = fopen($csvFile, "r")) !== FALSE) {
    ...
    $batch = [];
    while (($data = fgetcsv($handle, 10000, ",")) !== FALSE && $processed < 500) { // Limit for testing
        $processed++;
        
        try {
            // Skip if essential fields are missing
            if (empty($data[0]) || empty($data[3])) { // post_title, ID
                continue;
            }
            ...
            $errors++;
            echo "✗ Error processing product ID {$wpProduct['ID']}: " . $e->getMessage() . "\n";
        }
    }
