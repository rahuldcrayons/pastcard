...
class SmsUtility
{
    ...
    public static function order_placement($phone='', $order='')
    {
        ...
        $sms_body       = $sms_template->sms_body;
        $sms_body       = str_replace('[[user]]', json_decode($order->shipping_address)->name, $sms_body);
	...
	public static function update_tracking_code($phone='', $order='')
    {  
		$sms_template   = SmsTemplate::where('identifier','tracking_details_send')->first();
        $sms_body       = $sms_template->sms_body;
		$sms_body       = str_replace('[[user]]', json_decode($order->shipping_address)->name, $sms_body);
		$sms_body       = str_replace('[[order_code]]', $order->code, $sms_body);
        $sms_body       = str_replace('[[shipped]]', $order->delivery_partner, $sms_body);
		$sms_body       = str_replace('[[tracknum]]', $order->tracking_code, $sms_body);
        $template_id    = $sms_template->template_id;
        try {
            sendSMS($phone, env('APP_NAME'), $sms_body, $template_id);
        } catch (\Exception $e) {
	...
	public static function cod_otp_sms($code='', $phone='')
    {
        $sms_template   = SmsTemplate::where('identifier','phone_number_verification')->first();
        $sms_body       = $sms_template->sms_body;
        $sms_body       = str_replace('[[code]]', $code, $sms_body);
        $sms_body       = str_replace('[[site_name]]', 'COD', $sms_body);
        $template_id    = $sms_template->template_id;
        try {
            sendSMS($phone, env('APP_NAME'), $sms_body, $template_id);
        } catch (\Exception $e) {
