...
class SendSMSUtility
{
    public static function sendSMS($to, $from, $text, $template_id)
    {
        ...
        } elseif (OtpConfiguration::where('type', 'smsalert')->first()->value == 1) {
            try {
                $token = env('SMSALERT_APIKEY');
                $senderid = env('SMSALERT_SENDER_ID');
                Log::info("https://www.smsalert.co.in/api/push.json?apikey=".$token."&route=transactional&sender=".$senderid."&mobileno=".$to."&text=".rawurlencode($text));
                $ch = curl_init();
                curl_setopt_array($ch, array(
                    CURLOPT_URL => "https://www.smsalert.co.in/api/push.json?apikey=".$token."&route=transactional&sender=".$senderid."&mobileno=".$to."&text=".rawurlencode($text),
                    CURLOPT_RETURNTRANSFER => true,
                    CURLOPT_ENCODING => "",
                    CURLOPT_MAXREDIRS => 10,
                    CURLOPT_TIMEOUT => 0,
		...
		elseif (OtpConfiguration::where('type', 'sms4mrkt')->first()->value == 1) 
		{ 
			$params = array(
			   "Phno" => $to,
			   "Msg" => urlencode($text),
            ...
            $url='http://sms4mrkt.com/api/SmsApi/SendBulkApi';
