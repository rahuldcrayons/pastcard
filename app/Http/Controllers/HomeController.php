...
class HomeController extends Controller
{
    ...
    {
        // Cache featured categories for 24 hours
    ...
    public function videocompress()
    {
        $startDate = '2024-01-01'; //date('Y-m-d H:i:s');
        $endDate = date('Y-m-d H:i:s');
        $products = Product::whereBetween('created_at', [$startDate, $endDate])->orderBy('created_at', 'desc')->get();
        foreach ($products as $product) {
            $videoPath = uploaded_asset($product->videos);
            $name = basename($videoPath);
            $ext = pathinfo($videoPath, PATHINFO_EXTENSION);
            $outputPath = base_path('public/uploads/all/demo.' . $ext);
            ConvertVideo::dispatch($videoPath, $outputPath);
            File::delete($videoPath);
            rename(base_path('public/uploads/all/demo.' . $ext), base_path('public/uploads/all/' . $name));
            $size = filesize(base_path('public/uploads/all/' . $name));
            Upload::where('id', $product->videos)->update(['file_size' => $size]);
        }
        //
    }
    ...
    public function beaseller()
    {
        return view('frontend.beaseller');
    }
    public function login()
    {
        ...
        $user = Admin_setting::where('id', '1')->first();
        $key = $user->capcha;

        return view('frontend.user_login', array('key' => $key));
    }
    ...
    public function registration(Request $request)
    {
        ...
        if ($request->has('referral_code') && addon_is_activated('affiliate_system')) {
            try {
                ...
                // $affiliateController = new AffiliateController; // Commented out - addon not installed
                // $affiliateController->processAffiliateStats(...); // Commented out - addon not installed
            } catch (\Exception $e) {
            }
        }
        return view('frontend.user_registration', array('key' => $key));
    }
    ...
    public function userProfileUpdate(Request $request)
    {
        ...
        if ($seller) {
            ...
            if (gettype($request->delivery_partner) == 'array') {
                $seller->delivery_partner = json_encode($request->delivery_partner);
            }
            $seller->save();
        }
    ...
    public function product(Request $request, $slug)
    {
        ...
        if ($detailedProduct != null && $detailedProduct->published) {
            ...
            if ($detailedProduct->digital == 1) {
                ...
                if (Auth::check()) {
                    $orderd = Order::where('user_id', Auth::user()->id)->orderBy('id', 'desc')->first();
                    if ($orderd) {
                        $order = Order::findOrFail($orderd->id);
                        $pidd = 0;
                        foreach ($order->orderDetails as $key => $orderDetail) {
                            $pidd = $orderDetail;
                        }
                        return view('frontend.product_details', compact('detailedProduct', 'pidd'));
                    } else {
    ...
    public function show_product_upload_form(Request $request)
    {
        ...
        $conditions = collect(['Average', 'Poor', 'New', 'Excellent']);
        if (addon_is_activated('seller_subscription')) {
            if ($seller->seller_package && $seller->seller_package->product_upload_limit > $seller->user->products()->count()) {
                ...
                return view('frontend.user.seller.product_upload', compact('categories', 'conditions'));
            } else {
    ...
    public function show_product_edit_form(Request $request, $id)
    {
        ...
        $conditions = collect(['Average', 'Poor', 'New', 'Excellent']);
        $categories = Category::where('parent_id', 0)->where('digital', 0)->with('childrenCategories')->get();
        return view('frontend.user.seller.product_edit', compact('product', 'categories', 'tags', 'lang', 'conditions'));
    }
    ...
    public function variant_price(Request $request)
    {
        ...
        $max_limit = (!is_null($product->max_qty)) ? $product->max_qty : $product_stock->qty;
    ...
    public function about_us()
    {
        ...
        $page_type = 'about_us';
        return view("frontend.custom_page", compact('page', 'page_type'));
    }

    public function contact_us()
    {
        $page = Page::where('type', 'contact_us_page')->first();
