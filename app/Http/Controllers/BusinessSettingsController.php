...
class BusinessSettingsController extends Controller
{
 ...
 public function price_setting(Request $request)
    {
        $user = Admin_setting::where('id', '1')->first();
        $charge_normal= DB::table('shipping_types')->where('name','Normal Delivery')->pluck('price')->first();
        $charge_premium= DB::table('shipping_types')->where('name','Fast Delivery')->pluck('price')->first();
        CoreComponentRepository::instantiateShopRepository();
        CoreComponentRepository::initializeCache();
    	 return view('backend.setup_configurations.price_setting', compact('user','charge_normal','charge_premium'));
    }
    ...
    public function google_recaptcha(Request $request)
    {
        ...
        $user = Admin_setting::where('id', '1')->first();
        $key = $user->capcha;
        return View('backend.setup_configurations.google_configuration.google_recaptcha', array('key' => $key));
    }
    ...
    public function price_update(Request $request)
    {
         $user = Admin_setting::where('id', '1')->first();
                ...
                $user->percent_premium = $request->input('percent_premium');
                $user->save();
                DB::table('shipping_types')->where('name', 'Normal Delivery')->update(['price' => $request->input('charge_normal')]);
                DB::table('shipping_types')->where('name', 'Fast Delivery')->update(['price' => $request->input('charge_premium')]);
    ...
    public function google_recaptcha_update(Request $request)
    {
        ...
        $user = Admin_setting::where('id', '1')->first();
        $user->capcha = $request->input('CAPTCHA_KEY');
        $user->save();
    ...
    public function update(Request $request)
    {
        foreach ($request->types as $key => $type) {
            ...
            if($type == 'gst_no'){
                $this->overWriteEnvFile('GST_NO', $request[$type]);
            }
            if($type == 'timezone'){
