...
class CheckoutController extends Controller
{
    ...
    public function checkout_done($combined_order_id, $payment)
    {
        ...
        if($combined_order->include_wallet==1)
        {
            $included_wallet_balance = 0;
            $included_wallet_balance = Auth::user()->balance;
            $user = User::findOrFail(Auth::user()->id);
    if($user) {
         $user->balance -= $included_wallet_balance;
         $wallet = new Wallet;
         $wallet->user_id = $user->id;
         $wallet->amount = $included_wallet_balance;
    ...
    public function get_membership_info(Request $request)
    {
        $is_premium_buy = isset($request->premium) ? 1 : 0;
        ...
        $shipping_info = Address::where('id', 1)->first();
        ...
        // dd($carts,$shipping_info,$total,$coin,$shipping_final,$shipping_type);
        return view('frontend.payment_select', compact('carts', 'shipping_info', 'total', 'coin', 'shipping_final', 'shipping_type', 'is_premium_buy'));
    }
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        if (Auth::user()->is_premium == 1) {
            $shipping_types = DB::table('shipping_types')->where('for_premium', 1)->pluck('name');
        }
        return view('frontend.delivery_info', compact('carts', 'shipping_types'));
    }
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            ...
            // $shipping_charge=($request['shipping_type']=='fast_delivery')?150:70;
            foreach ($carts as $key => $cartItem) {
                $product = \App\Models\Product::find($cartItem['product_id']);
                $tax += $cartItem['tax'] * $cartItem['quantity'];
                $subtotal += $cartItem['price'] * $cartItem['quantity'];
            ...
            $total = $subtotal + $tax + $shipping_charge;
            // dd($subtotal,$tax,$shipping_charge);
            $is_premium_buy = 0;
            return view('frontend.payment_select', compact('carts', 'shipping_info', 'total', 'coin', 'shipping_final', 'shipping_type', 'is_premium_buy'));
        } else {
    ...
    public function store_delivery_info_membership(Request $request)
    {


        // $product = \App\Models\Product::find(9100);
        ...
        if ($carts && count($carts) > 0) {
            ...
            $total = $subtotal + $tax + $shipping_charge;
            // dd($subtotal,$tax,$shipping_charge);
            return view('frontend.payment_select_membership', compact('carts', 'shipping_info', 'total', 'coin', 'shipping_final', 'shipping_type'));
        } else {
            flash(translate('Your Cart was empty'))->warning();
            return redirect()->route('home');
        }
